{% test assert_vera_lessthan(model, compare_to_table, date_column, distinct_column, first_interval, max_interval, first_interval_value, second_interval_value, time_fraction, compare_table_is_snapshot=FALSE, remove_n_outlier=0) %}
...
validate_first_interval as (
    select
        *,
        case
        when abs(diff_row_count) > {{ first_interval_value }} then
            FALSE
        else
            TRUE
        end as draft_pass
...
validate_second_interval as (
    select
        *,
        case
        when abs(diff_row_count) > {{ second_interval_value }} then
            FALSE
        else
            TRUE
        end as draft_pass
